...
df_alb = pd.read_csv(r"C:\repos\cohen\data\CohenAlbums.txt", sep="|")
dfi = pd.read_csv("C:/repos/cohen/data/CohenList.txt", sep="|")
df = dfi.dropna()
...
def benchmarkmodels_layout(fig):
    ...
    fig.update_layout(margin={"t": 65, "b": 20, "r": 0, "l": 0, "pad": 0})
...
#
df_albumrank1 = (
    df.groupby("Album 1")["Title 1"]
    .count()
    .to_frame()
    .reset_index()
    .sort_values("Title 1", ascending=False)
)
#
...
df_albumrank2 = (
    df.groupby("Album 2")["Title 2"]
    .count()
    .to_frame()
    .reset_index()
    .sort_values("Title 2", ascending=False)
)
...
df_long_year["Man"] = df_long_year["Man"].map({"Year 1": "AP", "Year 2": "Haakon"})
df_long_title = pd.melt(
    df,
    id_vars="Position",
    value_vars=[
        "Title 1",
...
df_long_title["Man"] = df_long_title["Man"].map({"Title 1": "AP", "Title 2": "Haakon"})
df_long_title

#
df_long_album = pd.melt(
    df,
    id_vars="Position",
    value_vars=[
        "Album 1",
...
df_long_album["Man"] = df_long_album["Man"].map({"Album 1": "AP", "Album 2": "Haakon"})
df_long_album
...
df_long1 = pd.merge(
    ...
    df_long_title,
    how="left",
    left_on=["Position", "Man"],
    right_on=["Position", "Man"],
)
df_long1
...
app.layout = dbc.Container(
    dbc.Row(
        [
            html.H1("Tower of Songs", style={"text-align": "center"}),
            dbc.Row(
                [
                    dbc.Col(
                        [
                                    ...
                                    dbc.Col(
                                        [
                                            ...
                                            dash_table.DataTable(
                                                id="tbl_1",
                                                data=df[["Title 1", "Album 1"]].to_dict(
                                                    "records"
                                                ),
                                                columns=[
                                                    {"name": i, "id": i}
                                ...
                                [
                                    dbc.Col(
                                        [
                                            ...
                                            dash_table.DataTable(
                                                id="tbl_5",
                                                data=df_album_forgotten1[
                                                    ["Forgotten1"]
                                                ].to_dict("records"),
                                                columns=[
                                                    {"name": i, "id": i}
                                                    for i in df_album_forgotten1.columns
                                                ],
                                                style_cell={
                                                    "backgroundColor": "#2c2c2c",
                                                    "color": "white",
                                ...
                                [
                                    dbc.Col(
                                        [
                                            ...
                                            dash_table.DataTable(
                                                id="tbl_3",
                                                data=df[["Title 2", "Album 2"]].to_dict(
                                                    "records"
                                                ),
                                                columns=[
                                                    {"name": i, "id": i}
                                ...
                                [
                                    dbc.Col(
                                        [
                                            ...
                                            dash_table.DataTable(
                                                id="tbl_6",
                                                data=df_album_forgotten2[
                                                    ["Forgotten2"]
                                                ].to_dict("records"),
                                                columns=[
                                                    {"name": i, "id": i}
                                                    for i in df_album_forgotten2.columns
                                                ],
                                                style_cell={
                                                    "backgroundColor": "#2c2c2c",
                                                    "color": "white",
